...
from clients.validators import (
    passport_series_validator, passport_number_validator, tin_validator,
    zip_code_validator
)
...
class BasePerson(BaseCreatedAtUpdatedAtModel):
    name = models.CharField('Имя', max_length=s.MAX_NAME_LENGTH)
    surname = models.CharField('Фамилия', max_length=s.MAX_NAME_LENGTH)
    patronymic = models.CharField('Отчество', max_length=s.MAX_NAME_LENGTH)
...
class Job(BaseCreatedAtUpdatedAtModel):
    ...
    dateEmp = models.DateField('Дата трудоустройства', blank=True, null=True)
    dateDismissal = models.DateField('Дата увольнения', blank=True, null=True)
...
class Passport(BaseCreatedAtUpdatedAtModel):
    ...
    giver = models.CharField(
        'Кем выдан', max_length=s.MAX_GIVER_LENGTH)
    ...
    def __str__(self):
        return (f"Паспорт {self.series} {self.number}, Выдан {self.giver}, "
...
class Client(BasePerson):
    ...
    livingAddress = models.OneToOneField(
        Address, on_delete=models.CASCADE, related_name='living_person')
    regAddress = models.OneToOneField(Address, on_delete=models.CASCADE)
    jobs = models.ManyToManyField(Job, related_name='emploee')
    curWorkExp = models.IntegerField(
        'На текущем месте работы стаж', blank=True)
